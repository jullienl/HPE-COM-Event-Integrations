...
# Usage: IMAGE=<ecr-uri>:1.0.2 ACCESS_ROLE_ARN=<role> INSTANCE_ROLE_ARN=<role> ./deploy-relay-aws.sh
#
set -euo pipefail
...
GHCR_IMAGE="${GHCR_IMAGE:-ghcr.io/jullienl/com-event-relay:1.0.2}"   # upstream (published by CI)
IMAGE="${IMAGE:?Set IMAGE to your private ECR URI, e.g. <acct>.dkr.ecr.<region>.amazonaws.com/com-event-relay:1.0.2}"
