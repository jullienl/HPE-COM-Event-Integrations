...
- **Docker** on that host. The **Compose plugin** is additionally required for
  ...
  ```
- **Published Bridge image:** `ghcr.io/jullienl/com-event-bridge:1.0.2`.
...
```bash
docker pull ghcr.io/jullienl/com-event-bridge:1.0.2
...
docker run -d --name com-event-bridge --restart unless-stopped \
  ...
  ghcr.io/jullienl/com-event-bridge:1.0.2
```
