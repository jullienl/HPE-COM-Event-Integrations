...
- `adapters`   — target integrations (OBM, ServiceNow, Splunk, generic webhook)
                 selected by the `TARGET` env var via `get_adapter()`.

Both consumers depend on this package, so a fix to a mapping or adapter is made
once, here.
"""

from .normalize import CanonicalEvent, normalize
