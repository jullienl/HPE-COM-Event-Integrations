...
| 7 | **De-duplication** — SQLite TTL store, keyed on `dedup_key` **+ adapter name** | ✅ | — | ✅ | [core/dedup.py](com-event-core/com_event_core/dedup.py#L23) (`already_done`/`mark_done`) |
...
| 9 | **Target forwarding** — `get_adapters()` + `deliver()` (fan-out, per-adapter dedup, partial-failure retry) | ✅ | — | ✅ | [core/deliver.py](com-event-core/com_event_core/deliver.py) + [core/adapters/](com-event-core/com_event_core/adapters/__init__.py#L36); called in [bridge/app.py](com-event-bridge/bridge/app.py) · [shim/worker.py](com-event-relay/shim/worker.py) |
...
| `dedup` | `DedupStore` — `already_done()` / `mark_done()` (per-adapter, SQLite TTL) | [dedup.py](com-event-core/com_event_core/dedup.py) |
| `adapters` | `get_adapters()` + the 17 target adapters (`obm`, `servicenow`, `opsramp`, `halo`, `splunk`, `github`, `slack`, `teams`, `jira`, `pagerduty`, `sentinel`, `datadog`, `elastic`, `bmc_helix`, `dynatrace`, `grafana`, `webhook`) | [adapters/](com-event-core/com_event_core/adapters) |
| `deliver` | `deliver()` fan-out + `PartialDeliveryError` (one event → one or many adapters) | [deliver.py](com-event-core/com_event_core/deliver.py) |
| `secrets` | `get_secret()` — resolve a secret from `<name>_FILE` (vault/CSI/Docker/systemd file) or env | [secrets.py](com-event-core/com_event_core/secrets.py) |
